...
function blockno(bb){
    if(block!=bb){
        ...
        for(var b=0;b<blockstoadd.length;b++){
            ...
            polys[b] = this.patcher.newdefault(10+180*x, 40*y, "poly~","blank.audio",voices);
            controls[ii] = this.patcher.newdefault(70+180*x, 40*y+25, "sel","getvoice");
            ii++;
            controls[ii] = this.patcher.newdefault(70+180*x, 40*y+50, "message")
            controls[ii].message("set", "voice_offset",voiceoffset+paramoffsets[b],",","voice_is",(voiceoffset+paramoffsets[b])/MAX_PARAMETERS);
            ii++;
            controls[ii] = this.patcher.newdefault(70+180*x, 40*y+75, "prepend","setvalue",0);
        ...
        for(var c=0;c<connectionstoadd.length;c++){
            ...
            if((ftyp=="audio")&&(ttyp=="audio")){
                if(fb<0){
                    this.patcher.connect(audio_ins[-1-fb],0,polys[tb],tno);
                }else if(tb<0){
                    this.patcher.connect(polys[fb],fno,audio_outs[-1-tb],0);
                }else{
                    this.patcher.connect(polys[fb],fno,polys[tb],tno);
                }
            }
        }
    }
