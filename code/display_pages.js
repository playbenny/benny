				...
				if((from_multi)&&(to_multi>0)){ 
					if(corners[0]){ //many-corner(many)-blob-  then later i'll do -?corner(many)?-many
						for(i=0;i<from_list.length;i++){
							...
							//from_corner[2] = (1 - from_list[i]+fconx+blob_position[2])*0.5;
							for(t=0;t<3;t++){
					...
					if(corners[1]){//blob-corner(many)-many
						for(i=0;i<to_list.length;i++){
							//to_pos[0] = tp + 0.5*to_list[i] + 0.25*tconx;
							//to_corner[2] = (1 - to_list[i]+tconx+blob_position[2])*0.5;
							for(t=0;t<3;t++){
					...
					if(corners[1]){	
						//if(to_type=="hardware") post("hw conn,",tconx);
						for(t=0;t<3;t++){
							bez_prep[0][t] = blob_position[t];
							bez_prep[1][t] = blob_position[t]+meanvector[t];
							bez_prep[2][t] = to_corner[t]-to_cornervector[t];
					...
					if(corners[1]){//blob-corner(many)-many
						for(i=0;i<to_list.length;i++){
							//to_pos[0] = tp + 0.375 + 0.5 * to_list[i] + 0.25 * tconx;
							//to_corner[2] = (1 - to_list[i]+tconx+blob_position[2])*0.5;
							for(t=0;t<3;t++){
								bez_prep[0][t] = blob_position[t];
								bez_prep[1][t] = blob_position[t]+meanvector[t];
...
function draw_topbar(){
	...
	mouse_click_actions[mouse_index] = show_cpu_meter;
...
//		redraw_flag.targets = [];
	...
	if(sidebar.mode == "edit_label"){
		...
		for(var i = 0;i<MAX_BLOCKS;i++){
			...
			click_info(cpu_select_block,-1,i);
			tx += wm;
...
// BLOCK SCOPES AND PARAMS #######################################################################################################
		...
		if(selected.block_count == 1){ // if 1 block selected
			...
			click_rectangle( sidebar.x, y_offset, sidebar.x+fontheight*(5.6/*-editbtn*/),fontheight+y_offset,mouse_index,1);
			mouse_click_actions[mouse_index] = set_sidebar_mode;
			mouse_click_parameters[mouse_index] = "block";
			mouse_click_values[mouse_index] = "";	
			mouse_index++;				
		...
		// BLOCK SETTINGS ##############################################################################################################
				...
				if((block_type == "audio")||(block_type == "hardware")){
					...
					click_rectangle( mainwindow_width-9-7.6*fontheight, y_offset, mainwindow_width-9-6.6*fontheight, fontheight+y_offset,mouse_index,1 );
					mouse_click_actions[mouse_index] = set_record_arm;
					mouse_click_parameters[mouse_index] = block;
					mouse_click_values[mouse_index] = -1;	
					mouse_index++;
				...
				click_info(open_patcher, block, -1);
				lcd_main.message("frgb" , block_colour);
...
//						post("scopes voicelist",sidebar.scopes.voicelist);
			...
			if(selected.block_count > 1){ 
				...
				if(recarmflag){
					...
					lcd_main.message("frgb" ,0,0,0);
					click_rectangle( sidebar.x + fontheight*3.7, y_offset, sidebar.x + fontheight*4.7, fontheight+y_offset,mouse_index,1 );
					mouse_click_actions[mouse_index] = arm_selected_blocks;
					mouse_click_parameters[mouse_index] = 0;
					mouse_click_values[mouse_index] = 1;	
					mouse_index++;
