...
9/2/25:
- HUGE PERFORMANCE IMPROVEMENTS! i'd been putting this off but it wasn't as bad as i feared (!) the 3d graphics now takes advantage of 'hardware instancing' which means the cpu overhead of large patches is vastly reduced. on my mid-range framework intel laptop with a particular largeish patch (30 blocks) from my live set the cpu usage/power draw drops by 30% despite it actually hitting a higher FPS than before. 
...
- there may be a few minor bugs left to work out from the transition to the new system but they should only affect rendering. switching views and back should always trigger a clean redraw.
2/2/25:
