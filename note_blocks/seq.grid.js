...
function fulldraw(){
	...
	for(r=0;r<v_list.length;r++){
		...
		cursors[r]=ph;
		for(c=maxl-1;c>=0;c--){			
			outlet(0,"custom_ui_element","data_v_scroll", sx+c*cw+x_pos,r*rh+y_pos,sx+(0.9+c)*cw+x_pos,(r+0.9)*rh+y_pos,255*(c==ph),63+192*((c>=s[r])&&(c<s[r]+l[r])),255*(c==ph),MAX_DATA*v_list[r]+1+c);
			if(!mini){
